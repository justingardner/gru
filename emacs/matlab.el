...
;; 10-May-99|2.2.2|~/modes/matlab.el|
;;
...
(defconst matlab-mode-version "2.2.2"
...
\\|for\\|global\\|if\\|otherwise\\|return\\|switch\\|try\\|while\\)\\>"
   ...
   ;;             "uimenu" "uitoolbar" "uitoggletool" "uipushtool"
...
(defvar matlab-really-gaudy-font-lock-keywords
  (append
   ...
   (list
    ...
    '("\\<\\(db\\(c\\(lear\\|ont\\)\\|down\\|mex\\|quit\\|st\\(a\\(ck\\|tus\\)\
\\|ep\\|op\\)\\|type\\|up\\)\\)\\>" (0 'bold)))
...
(defvar matlab-hilit19-patterns
  '(
    ("\\(^\\|[^%]\\)\\(%[ \t].*\\|%\\)$" 2 comment)
    ("\\(^\\|[;,]\\)[ \t]*\\(\
function\\|global\\|for\\|parfor\\|while\\|if\\|elseif\\|else\\|end\\(function\\)?\
...
(defconst matlab-block-beg-pre-if "function\\|for\\|parfor\\|while\\|if\\|switch\\|try"
