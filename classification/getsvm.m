...
%
%            try classifyTest to play with this function
%
%            You can also do a fisher linear disciriminant:
%
%            svm = getsvm(x1,x2,'fisher');
% 
%            The inversion of the covariance matrix for fisher can be done
%            either using pseudoinverse (pinv), backslash operator (mldivide) or diagonal elements only (diag).
%            Default is to use mldivide.
%  
%            svm = getsvm(x1,x2,'fisher','pinv');
%            
...
[alpha svm.quadprog.fval svm.quadprog.exitflag svm.quadprog.output svm.quadprog.lambda] = quadprog(H,f,[],[],Aeq,beq,zeros(1,n),ones(1,n)*C,[],optimset('Display','off','Algorithm','interior-point-convex'));
...
% get arguments
svm.inversionType = 'mldivide';
if isstr(svm.kernelargs)
  svm.inversionType = svm.kernelargs;
end
...
% now compute weights
if strcmp(svm.inversionType,'diag')
  % only use diagonal elements of covariance matrix to do inversion
...
elseif strcmp(svm.inversionType,'mldivide')
  ...
  % in which underconstrained weights get set to 0
  svm.w = Sw\(m1-m2)';
else
  disp(sprintf('(getsvm) Unknown inversion method for covariance matrix: %s. Should be pinv, diag or mldivide.',svm.inversionType));
  keyboard
end
