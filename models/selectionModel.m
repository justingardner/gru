...
% eachOrAll- Can be either 'each' or 'all'. Each means to let all CRF parameters float for all conditions. All means
%            to use the same parameters for all CRF functions but only allow the offset to shift. To not allow response
%            gain, you should set this to all.
%
% sigma parameters
%
...
%
function retval = selectionModel(CRF,CDF,varargin)
...
if m.setCRFMin
  ...
  for iCond = 1:m.numConditions
    ...
    r(end+1:end+m.numPeds(iCond)) = CRF.(m.conditions{iCond}).cued.data.r;
    % grab uncued responses
    if ~CRF.(m.conditions{iCond}).sameCuedUncued
      r(end+1:end+m.numPeds(iCond)) = CRF.(m.conditions{iCond}).uncued.data.r;
    end
  end
...
[argNames argValues args] = getArgs(args,{'verbose=1','dispFig=0','dispCRF=0','dispCDF=0','fitCDF=0','dispIter=1','dispCond=0','fitCRFCDF=0','recompute=0','saveName=[]','crossValidate=0','saveIter=0','dispSavedIter=0','reslog=2','fixedParams=[]','initParams=[]','reuseFig=1','noFig=0','savePathDir=[]','colors=[]','symbols=[]'});
...
getArgs(args,{'poolingRule=exponential','sigmaRatio=[]','sigmaForEachCond=0','eachOrAll=each','fixedK=0','fixedN=0','CRFInterpFun=nakarushton','randDistractors=0','CDFres=1','CRFres=1','normResByVar=0','filename=[]','sigmaCuedUncuedRatio=[]','sigmaCuedUncued=0','outputNonlin=0','outputNonlinFit=0','setCRFMin=1'});
