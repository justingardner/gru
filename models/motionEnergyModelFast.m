...
%             10x10cm stimulus at 57 cm) it is 60 times faster than with 
%             conv function - although less precise: difference in values 
%             in the order of magnitude of 1e-9.
%       date: 05/29/17
...
%   end
% end
% [steeve] : convolution can be replaced much faster fast Fourier
% tranform (e.g., for 100x100 pix; 10x10cm stimulus at 57 cm)
% it is 60 times faster than with conv - although less precise: 
% difference in values in the order of magnitude of 1e-9.
n = size(s,3) + size(f,3) - 1;
r = ifft(fft(s,n,3).*fft(f,n,3),[],3);
