...
%             to run the MLR pRF code. It is written so that it can be containerized.
%
%             homedir: A string that is the directory where this function will temporarily
%                      create (and then delete) an MLR directory for processing
%
...
%             doParallel: (default -1) Use every worker available from the
%               local cluster. Set to 0 or 1 for true serial execution, or
%               to an integer greater than 1 to request exactly that many
%               workers.
%             progressEvery: (default 0) Number of completed voxels between
%               aggregate progress lines. Zero selects an automatic cadence;
%               -1 disables aggregate progress.
%             diagnosticVoxelCount: (default 10) Number of stratified-random
%               fits to print in verbose voxel format during each scan.
%             diagnosticSeed: (default 5489) Reproducible private seed used
%               only to select diagnostic voxels.
...
%               of pRFv3 to see models that have been coded.
%
...
% Keep worker-level logging off for standalone runs by default.
params.pRFFit.verbose = 0;
params.pRFFit.progressEvery = flags.progressEvery;
params.pRFFit.diagnosticVoxelCount = flags.diagnosticVoxelCount;
params.pRFFit.diagnosticSeed = flags.diagnosticSeed;
params.pRFFit.numWorkers = flags.numWorkers;
...
% Put pRFv3 first without removing another plugin's directory tree. The
% activation helper verifies every shared runtime function used here, so a
% path conflict is reported before the standalone analysis begins.
try
  pRFv3ActivatePath;
catch pathException
  results.status = -1;
  results.errorString = sprintf( ...
    '(mlrRunPRF:checkPRFPath) Could not activate pRFv3: %s', ...
    pathException.message);
  return
end

% pRFv3ActivatePath has already verified that pRF and the complete runtime
% dependency set resolve inside the active pRFv3 tree, regardless of their
% exact subdirectories.
resolvedPRF = which('pRF');
disp(sprintf('(mlrRunPRF:checkPRFPath) Using pRF found at: %s',resolvedPRF));
...
% check arguments
getArgs(varargin,{'dispFigs=0','quickFit=0','doParallel=-1', ...
  'progressEvery=0','diagnosticVoxelCount=10','diagnosticSeed=5489', ...
  'canonicalParams=[5.4 5.2 10.8 7.35 0.35]','rfType=gaussian'},'verbose=1');

if ~(isnumeric(progressEvery) && isscalar(progressEvery) && ...
    isfinite(progressEvery) && (progressEvery == -1 || progressEvery >= 0))
  results.status = -1;
  results.errorString = '(mlrRunPRF) progressEvery must be -1 or a nonnegative number';
  return
end
if ~(isnumeric(diagnosticVoxelCount) && isscalar(diagnosticVoxelCount) && ...
    isfinite(diagnosticVoxelCount) && diagnosticVoxelCount >= 0)
  results.status = -1;
  results.errorString = '(mlrRunPRF) diagnosticVoxelCount must be a nonnegative number';
  return
end
if ~(isnumeric(diagnosticSeed) && isscalar(diagnosticSeed) && ...
    isfinite(diagnosticSeed) && diagnosticSeed >= 0)
  results.status = -1;
  results.errorString = '(mlrRunPRF) diagnosticSeed must be a nonnegative number';
  return
end

% Resolve standalone parallelism through the same noninteractive path used
% by the GUI analysis and uncached prefit construction.
if ~(isnumeric(doParallel) && isreal(doParallel) && isscalar(doParallel) && ...
    isfinite(doParallel) && doParallel == fix(doParallel) && ...
    (doParallel == -1 || doParallel >= 0))
  results.status = -1;
  results.errorString = '(mlrRunPRF) doParallel must be -1, 0, or a positive integer';
  return
end
if doParallel == -1
  requestedWorkers = pRFResolveNumWorkers;
elseif doParallel <= 1
  requestedWorkers = 1;
else
  requestedWorkers = double(doParallel);
end
flags.numWorkers = pRFResolveNumWorkers(requestedWorkers);
...
flags.progressEvery = progressEvery;
flags.diagnosticVoxelCount = diagnosticVoxelCount;
flags.diagnosticSeed = diagnosticSeed;
